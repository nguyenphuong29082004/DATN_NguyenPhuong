...
function buildFinalPrompt({ modelName, clothingDescription, prompt, negativePrompt, quality }: { modelName: string; clothingDescription: string; prompt?: string; negativePrompt?: string; quality?: string }) {
  ...
  if (quality === 'hd') finalPrompt += ', premium high-definition detail, crisp fabric texture, refined facial detail'
...
function derivePreparationReference(trainingData: unknown): string | null {
  if (typeof trainingData === 'string' && trainingData.trim()) return trainingData.trim()
  if (Array.isArray(trainingData)) {
    for (const entry of trainingData) {
      if (typeof entry === 'string' && entry.trim()) return entry.trim()
      if (!entry || typeof entry !== 'object') continue
      const record = entry as Record<string, unknown>
      const candidate = record.lora_weights || record.weightsUrl || record.weightUrl || record.modelUrl || record.referenceUrl || record.imageUrl || record.url
...
async function deductCredits(supabaseAdmin: any, userId: string, amount: number, metadata: Record<string, unknown>) {
  const { data, error } = await supabaseAdmin.rpc('deduct_credits', { p_user_id: userId, p_amount: amount, p_reason: 'try_on_generation', p_metadata: metadata })
  if (error || !data?.success) throw new Error(error?.message || data?.error || 'Insufficient credits')
}

async function refundCredits(supabaseAdmin: any, userId: string, amount: number, metadata: Record<string, unknown>) {
  await supabaseAdmin.rpc('add_credits_secure', { p_user_id: userId, p_amount: amount, p_reason: 'refund_failed_try_on', p_metadata: metadata })
}
...
async function resolvePreparedMarketplaceImage({ supabaseAdmin, model, userId, replicateApiToken }: { supabaseAdmin: any; model: any; userId: string; replicateApiToken: string }) {
  ...
  if (existingAsset?.prepared_image_url && existingAsset?.source_signature === sourceSignature) {
    ...
    await supabaseAdmin.from('try_on_temp_assets').update({ last_checked_at: nowIso, last_verified_at: stillValid ? nowIso : existingAsset.last_verified_at, status: stillValid ? 'ready' : 'failed', error_message: stillValid ? null : 'Prepared image URL expired or is no longer reachable', updated_at: nowIso }).eq('id', existingAsset.id)
    if (stillValid) return { modelImageUrl: existingAsset.prepared_image_url, reusedPreparedImage: true }
  }

  await supabaseAdmin.from('try_on_temp_assets').upsert({ user_id: userId, model_id: model.model_id, prepared_image_url: existingAsset?.prepared_image_url || 'pending://preparing', provider: 'replicate', status: 'preparing', source_signature: sourceSignature, error_message: null, updated_at: nowIso }, { onConflict: 'user_id,model_id' })

  const preparationReference = derivePreparationReference(model.training_data)
  if (!preparationReference) {
    await supabaseAdmin.from('try_on_temp_assets').update({ status: 'failed', error_message: 'Marketplace model is missing training_data reference for try-on preparation', updated_at: nowIso }).eq('user_id', userId).eq('model_id', model.model_id)
  ...
  const preparationResponse = await fetch(`https://api.replicate.com/v1/models/${DEFAULT_PREP_MODEL}/predictions`, {
    method: 'POST',
    headers: { Authorization: `Token ${replicateApiToken}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ input: { prompt: PREP_SYSTEM_PROMPT, aspect_ratio: '3:4', lora_weights: preparationReference, lora_scale: 0.8, prompt_upsampling: false, disable_safety_checker: true } }),
  })
  ...
  if (!preparationResponse.ok) {
    ...
    await supabaseAdmin.from('try_on_temp_assets').update({ status: 'failed', error_message: message, updated_at: nowIso }).eq('user_id', userId).eq('model_id', model.model_id)
  ...
  if (!preparedImageUrl && predictionId) {
    for (let attempt = 0; attempt < 15 && !preparedImageUrl; attempt += 1) {
      ...
      const pollResponse = await fetch(`https://api.replicate.com/v1/predictions/${predictionId}`, { headers: { Authorization: `Token ${replicateApiToken}` } })
      if (!pollResponse.ok) continue
      const pollData = await pollResponse.json()
      if (pollData.status === 'failed' || pollData.status === 'canceled') {
        const message = pollData.error || `Preparation ${pollData.status}`
        await supabaseAdmin.from('try_on_temp_assets').update({ status: 'failed', prediction_id: predictionId, error_message: message, updated_at: new Date().toISOString() }).eq('user_id', userId).eq('model_id', model.model_id)
        throw new Error(`Failed to prepare marketplace model: ${message}`)
      }
  ...
  if (!preparedImageUrl) {
    await supabaseAdmin.from('try_on_temp_assets').update({ status: 'failed', prediction_id: predictionId, error_message: 'Preparation did not produce an output image', updated_at: new Date().toISOString() }).eq('user_id', userId).eq('model_id', model.model_id)
  ...
  await supabaseAdmin.from('try_on_temp_assets').upsert({ user_id: userId, model_id: model.model_id, prepared_image_url: preparedImageUrl, provider: 'replicate', status: 'ready', source_signature: sourceSignature, prediction_id: predictionId, error_message: null, expires_at: expiresAt, last_checked_at: updatedAt, last_verified_at: updatedAt, updated_at: updatedAt }, { onConflict: 'user_id,model_id' })
  return { modelImageUrl: preparedImageUrl, reusedPreparedImage: false }
}
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!supabaseUrl || !supabaseAnonKey || !supabaseServiceRoleKey || !replicateApiToken) throw new Error('Missing environment configuration')
    ...
    if (authError || !user) return new Response(JSON.stringify({ error: 'Unauthorized' }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' }, status: 401 })

    const { prompt, negativePrompt, modelId, aiCharacterId, wardrobeItemId, garmentUrl, modelImageUrl: clientModelImageUrl, width, height, seed, quality, format } = await req.json()
    const hasModelId = !!modelId
    const hasAiCharacterId = !!aiCharacterId
    if (hasModelId === hasAiCharacterId) throw new Error('Try-on requires exactly one source: modelId or aiCharacterId')

    const supabaseAdmin = createClient(supabaseUrl, supabaseServiceRoleKey)
    const { clothingDescription, resolvedGarmentUrl } = await resolveGarmentDetails(supabaseAdmin, wardrobeItemId || null, garmentUrl || null)
    if (!resolvedGarmentUrl) throw new Error('A valid garment image is required. Please select or upload a clothing item.')
    ...
    if (aiCharacterId) {
      ...
      const { data: aiChar } = await supabaseAdmin.from('ai_models').select('id, name, preview_images').eq('id', aiCharacterId).single()
      if (!aiChar) throw new Error('Selected AI character could not be found')
    ...
    const finalPrompt = buildFinalPrompt({ modelName, clothingDescription, prompt, negativePrompt, quality })
    const cost = quality === 'hd' ? 10 : DEFAULT_TRY_ON_COST

    await deductCredits(supabaseAdmin, user.id, cost, { prompt: finalPrompt.substring(0, 50), wardrobe_item_id: wardrobeItemId || null, source_model_type: sourceModelType, reused_prepared_image: reusedPreparedImage })
    ...
    try {
      const replicateResponse = await fetch(`https://api.replicate.com/v1/models/${DEFAULT_TRY_ON_MODEL}/predictions`, {
        method: 'POST',
        headers: { Authorization: `Token ${replicateApiToken}`, 'Content-Type': 'application/json' },
        body: JSON.stringify({ input: { prompt: finalPrompt, image_input: [modelImageUrl, resolvedGarmentUrl], aspect_ratio: resolveAspectRatio(width, height), output_format: format === 'png' ? 'png' : 'jpg' }, webhook: webhookUrl, webhook_events_filter: ['completed'] }),
      })
      ...
      await refundCredits(supabaseAdmin, user.id, cost, { error: apiError.message, source_model_type: sourceModelType, stage: 'final_try_on' })
      throw new Error(`Failed to start try-on generation: ${apiError.message}`)
    }

    try {
      const { data: generation, error: genError } = await supabaseAdmin.from('generations').insert({
        user_id: user.id,
        model_id: modelId || null,
        ai_model_id: sourceModelType === 'ai_character' ? resolvedAiModelId : null,
        type: 'quick_shoot',
        generation_type: 'try-on',
        prompt_text: finalPrompt,
        parameters_json: { original_prompt: prompt, negative_prompt: negativePrompt, wardrobe_item_id: wardrobeItemId, ai_character_id: aiCharacterId, width, height, seed, quality: quality || 'standard', format: format || 'png', source_model_type: sourceModelType, reused_prepared_image: reusedPreparedImage, processing_stage: 'applying_clothing', ui_seed_supported: false, ui_negative_prompt_applied_via_prompt_text: !!negativePrompt, ui_quality_applied_via_prompt_text: quality === 'hd' },
        credits_used: cost,
        api_cost: cost,
        status: 'processing',
      ...
      return new Response(JSON.stringify({ success: true, generation }), { headers: { ...corsHeaders, 'Content-Type': 'application/json' } })
    } catch (dbError: any) {
      await refundCredits(supabaseAdmin, user.id, cost, { error: dbError.message, replicate_job_id: predictionId, stage: 'db_insert' })
